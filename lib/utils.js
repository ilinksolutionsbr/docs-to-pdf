...
function generateTocHtml(headers, tocTitle) {
    ...
    const toc = headers
        .map((header) => `<li class="toc-item toc-item-${header.level}" style="margin-left:${(header.level - 1) * 30}px"><a href="#${header.id}" class=${header.level == 2 ? 'style="font-weight:bold"' : ''}>${header.header}</a></li>`)
        .join('\n');
