...
function generateTocHtml(headers, tocTitle) {
    ...
    const toc = headers
        .map((header) => `<li class="toc-item toc-item-${header.level}" style="margin-left:${(header.level - 1) * 10}px"><p>${(header.level == 2 ? '.' : header.level == 3 ? '......' : header.level == 4 ?? '............')}</p><a href="#${header.header}</a></li>`)
        .join('\n');
