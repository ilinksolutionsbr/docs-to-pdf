...
function generateTocHtml(headers, tocTitle) {
    ...
    const toc = headers
        .map((header) => `<li class="toc-item toc-item-${header.level}" style="margin-left:${(header.level - 1) * 20}px"><a href="#${header.id}"><span>${header.level == 1 ? '' : header.level == 2 ? '.......' : '............'}</span>${header.header}</a></li>`)
        .join('\n');
